...
public class CrearTripulanteHandler_Test {
	...
	public void Handle_Ok() throws HttpException {
		...
		//ELIMINAR
		EliminarTripulanteCommand commandDelete = new EliminarTripulanteCommand(
			tripulanteDtoTest.Key
		);
		when(tripulanteRepository.FindByKey(tripulanteDtoTest.Key))
			.thenReturn(tripulanteTest);
		EliminarTripulanteHandler handlerDelete = new EliminarTripulanteHandler(
			tripulanteFactory,
			tripulanteRepository,
			unitOfWork
		);
		when(tripulanteRepository.Delete(tripulanteTest))
			.thenReturn(tripulanteTest);
		...
		EliminarTripulanteCommand commandDelete2 = new EliminarTripulanteCommand(
			UUID.randomUUID()
		);
		when(tripulanteRepository.FindByKey(UUID.randomUUID()))
			.thenReturn(tripulanteTest);
		EliminarTripulanteHandler handlerDelete2 = new EliminarTripulanteHandler(
			tripulanteFactory,
			tripulanteRepository,
			unitOfWork
		);
