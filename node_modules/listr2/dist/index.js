...
var _DefaultRenderer = class {
	...
	createRender(options) {
		...
		if (
			options.bottomBar &&
			(renderBottomBar == null ? void 0 : renderBottomBar.trim().length) >
				0
		) {
		...
		if (!((_a2 = this.options) == null ? void 0 : _a2.lazy)) {
			this.id = setInterval(() => {
				this.spinnerPosition =
					++this.spinnerPosition % this.spinner.length;
				updateRender();
			}, 100);
		}
			...
			if (task.isEnabled()) {
				...
				if (
					...
					if (
						...
						output = [
							...output,
							this.dumpData(task, level, "error"),
						];
					} else if (
						task.isSkipped() &&
						this.getSelfOrParentOption(task, "collapseSkips") ===
				...
				if (task == null ? void 0 : task.output) {
					if (
						...
						if (!this.bottomBar[task.id]) {
							...
							const bottomBar =
								this.getTaskOptions(task).bottomBar;
						...
						if (
							!((_b =
								(_a2 = this.bottomBar[task.id]) == null
									? void 0
									: _a2.data) == null
								? void 0
								: _b.some((element) =>
				...
				if (
					...
					(task.isPending() ||
						...
						(task.isCompleted() &&
							this.getSelfOrParentOption(task, "collapse") ===
								false &&
							!task.subtasks.some(
								(subtask) =>
									subtask.rendererOptions.collapse === true
							)) ||
						task.subtasks.some(
							(subtask) =>
								subtask.rendererOptions.collapse === false
						) ||
						task.subtasks.some((subtask) => subtask.hasFailed()) ||
						task.subtasks.some((subtask) =>
							subtask.hasRolledBack()
						))
				) {
	...
	formatString(str, icon, level) {
		...
		switch (this.options.formatOutput) {
			case "truncate":
				parsedStr = str.split(EOL).map((s, i) => {
					return cliTruncate(
						this.indentMultilineOutput(s, i),
						columns
					);
				});
				break;
		...
		return indentString(
			parsedStr.join(EOL),
			level * this.options.indentation
		);
	}
	...
	getSymbol(task, data = false) {
		...
		if (task.isPending() && !data) {
			return ((_a2 = this.options) == null ? void 0 : _a2.lazy) ||
				...
				: colorette_default.yellowBright(
						this.spinner[this.spinnerPosition]
				  );
	...
	constructor(tasks, options) {
		...
		this.eventTypeRendererMap = {
			["SUBTASK" /* SUBTASK */]: (task) => {
				if (task.hasTitle()) {
					this.log(
						`${colorette_default.blue(figures.pointer)} ${
							task.title
						}`
					);
				}
			...
			["STATE" /* STATE */]: (task) => {
				if (task.isCompleted() && task.hasTitle()) {
					this.log(
						`${colorette_default.green(figures.tick)} ${task.title}`
					);
				}
			},
			...
			["MESSAGE" /* MESSAGE */]: (task, event) => {
				if (event.data.error) {
					...
					const title = _SimpleRenderer.formatTitle(task);
					const skip =
						task.title !== event.data.skip
							? `: ${event.data.skip}`
							: "";
					this.log(
						`${colorette_default.yellow(
							figures.arrowDown
						)}${title} [${colorette_default.yellow(
							`skipped${skip}`
						)}]`
					);
...
var Task = class extends Subject {
	constructor(listr, tasks, options, rendererOptions) {
		...
		this.skip =
			(_d = (_c = this.tasks) == null ? void 0 : _c.skip) != null
				? _d
				: false;
		this.enabledFn =
...
async function createPrompt(options, settings) {
	...
	if (!Array.isArray(options)) {
		options = [
			__spreadProps(__spreadValues({}, options), {name: "default"}),
		];
			...
			Object.assign(option, {
				stdout:
					this instanceof TaskWrapper
						? (_a2 = settings == null ? void 0 : settings.stdout) !=
						  null
							? _a2
							: this.stdout()
						: process.stdout,
